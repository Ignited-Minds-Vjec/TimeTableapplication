...
class TimeTableApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, Developer, AutoMode):
    ...
    def autojoineron(self):
        self.show_frame(AutoMode)
        t = time.ctime()
        l = t.split(" ")
        day = l[0]
        k = days[day]
        now = l[4]
        l = now.split(":")
        sec = int(l[2]) + (int(l[1]) * 60) + (int(l[0]) * 60 * 60)
        ...
        if day == "Sat" or day == "Sun":
            ...
            for i in range(6):
                atl = get_keyFriday(i)
                k = atl.split(":")
                tsec = int(k[2]) + (int(k[1]) * 60) + (int(k[0]) * 60 * 60)
                if tsec > sec:
                    rem = tsec - sec
                    break
            for i in range(6):
                ...
                if atl <= now < ath:
                    ...
                    classes = S7_CSE_tt[k][i]
                    if classes == "NULL":
                        messagebox.showinfo("Choice Unavailable", "Multiple Options available\nExiting Auto Mode ")
                        # stop automode
                    else:
                        webbrowser.open_new(S7_CSE_links[classes])
            if flag == 0:
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        btn3 = tk.Button(self, text='Auto Mode', bd='5', command=lambda: controller.autojoineron())
...
class AutoMode(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label2.pack()
        print("hello")
        label3 = tk.Label(self, text="Auto Mode\nTurned ON",
                          foreground="green",
                          font=("Times New Roman", 10))
        label3.pack()

        button1 = tk.Button(self, text="Turn OFF",
                            command=lambda: controller.show_frame(StartPage))
        button1.pack(pady=5)
